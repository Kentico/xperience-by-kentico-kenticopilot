...
4. If any `Kentico.Xperience.*` package uses a `-preview` or `-prerelease` suffix and its base version matches the current version, check for a matching pre-release with `dotnet list <solution> package --outdated --include-prerelease` and update to the corresponding pre-release of the new version if one is available.
