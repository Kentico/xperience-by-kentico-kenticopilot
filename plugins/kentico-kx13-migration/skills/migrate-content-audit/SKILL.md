...
> `kentico-kx13-migration/src/` folder is present in your workspace.

Do **not** proceed to pre-flight or CLI execution without the solution files.

### 2. Pre-flight Checks

1. Read `kentico-kx13-migration/src/KX13.ContentAuditor.CLI/appsettings.development.json`
   (or `appsettings.json`). Verify that `ConnectionStrings.ConnectionString` is
   non-empty. If missing, ask the user to configure it.
2. Build: `dotnet build kentico-kx13-migration/src/KX13.ContentAuditor.CLI -c Release -q`  
