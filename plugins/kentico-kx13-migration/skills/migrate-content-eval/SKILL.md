...
Does NOT validate configuration — that is handled upstream by `migrate-content-appsettings` and `migrate-content-run` skills before migration runs.
...
Save to `<log-dir>/migrate-content-eval-report.html`.
...
| **FAIL (Config)** | Re-run `migrate-content-appsettings` skill to fix config → re-run migration with `--bypass-dependency-check` → re-evaluate. |
| **FAIL (Code)** | Re-run the relevant extension skill (`migrate-content-classes`, `migrate-content-widgets`, `migrate-content-items`, or `migrate-content-fields`) → rebuild extensions → re-run migration → re-evaluate. |
